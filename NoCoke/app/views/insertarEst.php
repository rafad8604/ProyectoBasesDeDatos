...
session_start();
$dni=$_POST['dni'];
$nombre_estudiante=$_POST['nomb_est'];
$apellido_estudiante=$_POST['ape_est'];
...
if($cantidad >= 1){
  ...
  $sql="INSERT INTO estudiantes(cod_est, dni, nomb_est, ape_est) values (nextval('estudiantes_codigo_seq'),'$dni','$nombre_estudiante','$apellido_estudiante')";
  $query = pg_query($sql);
  ...
  if($query){
    ...
    include_once  "estudiantes.php";
      echo '<script> Swal.fire({
          icon: "success",
          title: "Buen trabajo!",
          text: "Se ingreso de manera correcta el estudiante",
        });</script>';
    ...
    include_once  "estudiantes.php";
      echo '<script> Swal.fire({
          icon: "error",
          title: "Error al ingresar al estudiante",
        });</script>';
  }
